...
def color_match(img_a,img_b,blur_ksize=51):
    blur_a = cv2.GaussianBlur(img_a, (blur_ksize, blur_ksize), 0)
    # High frequency from B via Laplacian
    laplacian_b = cv2.Laplacian(img_b, cv2.CV_32F, ksize=3)
    # Combine
...
def main():
    ...
    parser.add_argument("--out_path", type=str, default="color_output.png", help="Path to save output plot")
